#!/usr/bin/env python3
"""从 Tushare 拉取公告并保存为统一 announcement_events 表。"""
...
from live.announcement_source import fetch_tushare_announcement_events, save_announcement_events
...
def _symbols_from_args(symbols_text: str, stock_pool: Path | None) -> list[str]:
    ...
    return [str(x) for x in pool["symbol"].dropna().astype(str).unique()]


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="从 Tushare 拉取公告事件表")
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = fetch_tushare_announcement_events(symbols, args.start, args.end)
