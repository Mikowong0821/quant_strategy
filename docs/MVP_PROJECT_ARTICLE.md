...
讲完「世界观」，我们回到「这个仓库长什么样」。下面这张图与九步表，是 **当前 Quant Strategy** 的工程契约总览；你加功能时，多半是加深某条边，而不是推翻入口。
...
与 `main.py` 编排顺序一致的九步（若主流程调整，以代码为准并同步更新本表）：
...
| 8 | 绩效汇总与终端打印。 |
| 9 | 可选写实验记录：`run_config.json`、`performance_summary.csv`、`rebalance_logs/*.csv`。 |
| 10 | 净值图、IC 图、权重图等；写盘时落 PNG 与 CSV。 |
...
**输出与配置：**绩效为研究常用简化口径（与券商对账单可能不一致，实盘需再对齐）。日常多改 `config.py`；Token 建议只走环境变量。打开 `persist_run_outputs` 后，系统会把行情 / 因子 / IC 缓存、配置快照、绩效汇总、调仓明细和图表一并保存，方便之后比较两次实验到底差在哪里。
