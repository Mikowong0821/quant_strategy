...
与 `main.py` 编排顺序一致的十二步（若主流程调整，以代码为准并同步更新本表）：
...
| 4 | 可选：缓存行情与面板，便于复现与对比。 |
| 5 | 计算 IC 及汇总；融合得分与当前融合逻辑一致后再算一条 FUSED 的 IC。 |
| 6 | 四列因子各跑一条回测。 |
| 7 | 融合得分（默认带 IC 列权，可关）再跑一条回测。 |
| 8 | 绩效汇总与终端打印。 |
| 9 | 构造股票池等权基准，计算超额收益、跟踪误差与信息比率。 |
| 10 | 根据调仓日志计算换手率、预估交易成本。 |
| 11 | 可选写实验记录：`run_config.json`、`performance_summary.csv`、`rebalance_logs/*.csv`、`turnover_logs/*.csv`。 |
| 12 | 净值图、超额净值图、换手图、IC 图、权重图等；写盘时落 PNG 与 CSV。 |
...
**输出与配置：**绩效为研究常用简化口径（与券商对账单可能不一致，实盘需再对齐）。日常多改 `config.py`；Token 建议只走环境变量。打开 `persist_run_outputs` 后，系统会把行情 / 因子 / IC 缓存、配置快照、绩效汇总、调仓明细、换手日志、净值图、超额净值图和换手图一并保存，方便之后比较两次实验到底差在哪里，也方便判断策略收益究竟来自股票池整体上涨、因子选择产生的相对收益，还是高换手带来的成本压力。
