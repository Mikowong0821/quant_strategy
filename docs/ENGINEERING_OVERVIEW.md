...
**定位**：A 股日频、研究向的 **「数据 → 因子面板 → IC → 单因子/融合回测 → 绩效与净值图 → 可选落盘」** 闭环；目录上预留 **实盘信号 / 模拟盘** 等扩展位。
...
- `main.py` 收集各因子与 `FUSED_ZSCORE` 净值后保存 **`output/nav_compare.png`**（`config.Settings.output_dir`）。
...
| 文档 | 内容 |
|------|------|
| [MVP_PROJECT_ARTICLE.md](./MVP_PROJECT_ARTICLE.md) | 项目正文：MVP 定位、流程图、九步表、数据/因子/IC/融合/回测与扩展方向。 |
